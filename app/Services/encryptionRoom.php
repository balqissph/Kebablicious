...

{
    ...
    public function __construct()
    {
        ...
        $this->aad = config('aes.aad', null);

        $this->aes->setKey($this->key);

        if ($this->aad) {
            $this->aes->setAAD($this->aad);
        }
    }
    ...
    public function encrypt(string $plain): string
    {
        ...
        // 1️⃣ Generate nonce baru tiap enkripsi (unik, 12 bytes disarankan untuk AES-GCM)
        $nonce = random_bytes(12);
        $this->aes->setNonce($nonce);
        ...
        // 3️⃣ Gabungkan hasil (cipher + tag + nonce)
        $jsonCipher = json_encode([
            'cipher' => base64_encode($cipher),
            'tag'    => base64_encode($tag),
...
//===============================================================================================================
        ...
        if (!isset($data['cipher'], $data['tag'], $data['nonce'])) {
            throw new \Exception('Invalid cipher format');
        }

        // 3️⃣ Set AES tag & nonce dari data
        $this->aes->setNonce(base64_decode($data['nonce']));
        $this->aes->setTag(base64_decode($data['tag']));
